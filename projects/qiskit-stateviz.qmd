...
## Two Spheres, One Spinor (merged from Spinor-Topology)

An interactive explainer, also merged in rather than kept as a third small standalone repo: why the Bloch sphere — the very picture `plot_bloch_multivector_interactive` above draws — is a simplification of a qubit's true state in ℂ², and why a spin-1/2 particle needs a full 720° (4π) rotation, not 360°, to return to its original state. A single slider drives two synced dials: a Bloch-sphere point (period 360°) and the SU(2) rotation parameter `θ/2` inside `R(θ) = exp(−iθ n̂·σ/2)` (period 720°), with live α/β amplitude readout computed from the actual rotation rather than approximated.

![One physical rotation, two periods — the −1 the Bloch sphere hides](../images/spinor-topology.svg)

The −1 phase picked up by a full 360° rotation is invisible on the Bloch sphere (global phase drops out of it entirely) but was measured directly, in Hilbert space, by the 1975 neutron interferometry experiments (Rauch, Zeilinger, Badurek, Wilfing, Bauspiess, and Bonse; independently, Werner, Colella, Overhauser, and Eagen) — real experimental confirmation, not just a formal curiosity, and the reason the Bloch-sphere picture used throughout the rest of this page is a real simplification, not a full description.
